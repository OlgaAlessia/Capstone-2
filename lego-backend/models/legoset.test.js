...
describe("create", function () {
  ...
  test("works", async function () {
    let legoSet = await LegoSet.create({...newLegoSet, num: '21319-1'});
    expect(legoSet).toEqual(newLegoSet);
    const found = await db.query("SELECT * FROM lego_sets WHERE num='21319-1'");
...
describe("remove", function () {
  test("works", async function () {
    ...
    const res = await db.query('SELECT num FROM lego_sets WHERE num=$1', [testLegoSetIds[0]]);
